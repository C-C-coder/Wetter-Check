...
def build_multi_location_update(tour):
   ...
   if live_lat and live_lon:
       # Nur hinzufügen, wenn es sich vom Start nennenswert unterscheidet (GPS Drift ignorieren > 0.01)
       if not start_lat or abs(float(live_lat) - float(start_lat)) > 0.01 or abs(float(live_lon) - float(start_lon)) > 0.01:
