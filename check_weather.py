...
def fetch_elevation(lat, lon):
    """Höhe eines einzelnen Punkts über Open-Meteos kostenlose Elevation-API.
    Wird für die Starthöhe gebraucht, wenn kein GPX-Track vorliegt - ohne
    Höhenangabe an beiden Enden ist keine Höhenzonen-Einteilung möglich."""
    res = http_json(f"https://api.open-meteo.com/v1/elevation?latitude={lat}&longitude={lon}", timeout=6)
    try:
...
def track_km_profile(punkte):
    """Kumulierte Distanz in km entlang des Tracks, gleicher Index wie das
    Aufwandsprofil aus track_effort_profile. Liefert die reale Streckenlaenge
    fuer die Beschriftung der Abschnitte (Aufwand und Distanz laufen bei
    Steilstuecken auseinander - "Mittelteil" nach Zeit kann kurz nach km sein)."""
    kum = [0.0]
    for i in range(1, len(punkte)):
        a, b = punkte[i - 1], punkte[i]
        kum.append(kum[-1] + calc_distance_km(a[0], a[1], b[0], b[1]))
    return kum
...
#
#  FIX (Entkopplung von Tempo/Tourform): Die Abschnitte liefen bisher nach
#  ZEITANTEIL (Naismith-Aufwand) entlang des GESAMTEN Tracks. Das setzt voraus,
#  dass die Person im geschaetzten Tempo unterwegs ist, den Track komplett
#  abgeht und nicht vorzeitig umkehrt. Bricht jemand die Tour ab, ist schneller
#  oder langsamer als angenommen, oder ist der GPX-Track eine Rundtour mit
#  Abstieg zurueck zum Start, verschieben sich die Zonen-ZEITEN
#  entsprechend falsch mit - im schlimmsten Fall landete "Gipfelbereich"
#  (immer der letzte Zeit-Abschnitt) teilweise auf dem Abstieg statt am Gipfel.
#
#  Jetzt: Zonen nach SEEHOEHE, nicht nach Zeit oder Position im Track. Ein
#  Punkt gehoert zu einer Zone, wenn seine Hoehe in deren Band liegt - egal ob
#  er beim Auf- oder Abstieg durchquert wird, egal wie schnell/langsam, egal ob
#  die Tour spaeter abgebrochen wird. Bei einer Rundtour durchquert man
#  dieselbe Hoehenzone zweimal (Hin- und Rueckweg) - das werden dann bewusst
#  ZWEI Eintraege mit demselben Namen und eigenem Zeitfenster, statt sie zu
#  einem einzigen, kaum noch aussagekraeftigen "irgendwann zwischen Start und
#  Ende"-Fenster zu verschmelzen.
# ==============================================================================
SEGMENT_NAMES_ELEV = ["Tal", "Unterer Teil", "Mittlerer Teil", "Oberer Teil", "Gipfel"]
SEGMENT_SAMPLES = 3       # Stützpunkte je Zonen-Durchquerung für die Nowcast-Abfrage
ELEV_BAND_TOLERANZ_M = 15 # Punkte knapp ausserhalb des Bands noch zuordnen, gegen Kantenrauschen
ELEV_MIN_SPANNE_M = 50    # Unterhalb dieser Gesamt-Hoehendifferenz sind Zonen nicht sinnvoll

# FIX (Grammatik): Die alten Namen ("Zustieg", "Mittelteil", "Gipfelbereich") passten
# alle zu "im " + Name. Die neue feste Namensliste tut das nicht mehr - "im
# Tal"/"im Gipfel" sind falsch (richtig: "im Tal" passt zufaellig, aber "am Gipfel"
# nicht), "im Unterer Teil" fehlt sogar die Deklination (richtig: "im unteren Teil").
# Deshalb Praeposition + Form pro Basisname fest hinterlegt statt "im " pauschal
# voranzustellen.
SEGMENT_PRAEP_FORM = {
    "Tal": "im Tal",
    "Unterer Teil": "im unteren Teil",
    "Mittlerer Teil": "im mittleren Teil",
    "Oberer Teil": "im oberen Teil",
    "Gipfel": "am Gipfel",
    "Gesamte Route": "auf der gesamten Route",
}


def segment_praep(name):
    """'Unterer Teil (Rückweg)' -> 'im unteren Teil (Rückweg)' - Suffixe wie
    '(Hinweg)'/'(Rückweg)' bleiben unangetastet, nur der Basisname wird über
    SEGMENT_PRAEP_FORM in die grammatikalisch passende Form gebracht."""
    basis = name.split(' (')[0]
    rest = name[len(basis):]
    return SEGMENT_PRAEP_FORM.get(basis, f"im {basis}") + rest


def build_segments(track_raw, start_lat, start_lon, peak_lat, peak_lon, anzahl=5, peak_alt=None):
    """Route in Höhenzonen zerlegen (siehe Kommentar oben), jeweils mit
    Höhenband, Distanz und Stützpunkten für die Wetterabfrage.

    Ohne GPX-Track (kein 'track' in der Tour hinterlegt) gibt es keine echten
    Wegpunkte - dann Luftlinie Start->Gipfel mit genau 2 Punkten. Die App
    speichert die Gipfelhöhe (peak_alt) beim Anlegen der Tour bereits in
    Firestore; die Starthöhe wird hier zusätzlich per Elevation-API geholt.
    Sind beide Höhen bekannt, funktioniert die Höhenzonen-Einteilung auch ohne
    Track - nur eben entlang der Luftlinie statt des echten Wegs, und ohne
    Hin-/Rückweg-Unterscheidung (die Luftlinie kennt keine Kehrtwende)."""
    tp = parse_track(track_raw)
    profil = track_effort_profile(tp) if tp else None
    # Für die km-Anzeige gemerkt, BEVOR der Luftlinie-Fallback profil überschreibt:
    # Luftlinien-km wären eine Kilometerangabe, die real falsch ist (echter Weg
    # hat Kehren, Umwege, Serpentinen) - besser gar keine Zahl als eine falsche.
    hat_echten_track = profil is not None
    ...
    if not profil:
        try:
            ...
            return []
        s_ele = fetch_elevation(s_lat, s_lon)
        try:
            p_ele = float(peak_alt) if peak_alt is not None else None
        except (TypeError, ValueError):
            p_ele = None

        if s_ele is not None and p_ele is not None:
            # Genug Zwischenpunkte, damit auch die mittleren Höhenzonen Treffer
            # bekommen - mit nur Start+Gipfel blieben "Unterer/Mittlerer/Oberer
            # Teil" leer, weil kein Punkt in ihrem Höhenband läge.
            LUFTLINIE_PUNKTE = 21
            tp = [(s_lat + (p_lat - s_lat) * (i / (LUFTLINIE_PUNKTE - 1)),
                   s_lon + (p_lon - s_lon) * (i / (LUFTLINIE_PUNKTE - 1)),
                   s_ele + (p_ele - s_ele) * (i / (LUFTLINIE_PUNKTE - 1)))
                  for i in range(LUFTLINIE_PUNKTE)]
            profil = [i / (LUFTLINIE_PUNKTE - 1) for i in range(LUFTLINIE_PUNKTE)]
        else:
            # Keine Höhe verfügbar (Elevation-API nicht erreichbar oder kein
            # peak_alt hinterlegt) - dann wie bisher ohne Höheninfo.
            tp = [(s_lat, s_lon, None), (p_lat, p_lon, None)]
            profil = [0.0, 1.0]

    km_profil = track_km_profile(tp)
    hoehen_vorhanden = [p[2] for p in tp if p[2] is not None]

    # Ohne (nennenswerte) Höhendaten sind Höhenzonen nicht sinnvoll - dann ein
    # einziger Abschnitt über die ganze Route statt fünf beliebiger Grenzen.
    if len(hoehen_vorhanden) < 2 or (max(hoehen_vorhanden) - min(hoehen_vorhanden)) < ELEV_MIN_SPANNE_M:
        return [{
            'name': 'Gesamte Route', 'von': 0.0, 'bis': 1.0,
            'punkte': [position_on_track(tp, profil, f) for f in (0.15, 0.5, 0.85)],
            'ele_min': int(min(hoehen_vorhanden)) if hoehen_vorhanden else None,
            'ele_max': int(max(hoehen_vorhanden)) if hoehen_vorhanden else None,
            'km_von': 0.0 if hat_echten_track else None,
            'km_bis': round(km_profil[-1], 1) if hat_echten_track else None,
        }]

    e_min, e_max = min(hoehen_vorhanden), max(hoehen_vorhanden)
    spanne = e_max - e_min
    namen = SEGMENT_NAMES_ELEV[:anzahl] if anzahl <= len(SEGMENT_NAMES_ELEV) else SEGMENT_NAMES_ELEV
    n = len(namen)

    segmente = []
    for i in range(n):
        band_von = e_min + spanne * (i / n)
        band_bis = e_min + spanne * ((i + 1) / n)

        idxs = [idx for idx, p in enumerate(tp)
                if p[2] is not None
                and band_von - ELEV_BAND_TOLERANZ_M <= p[2] <= band_bis + ELEV_BAND_TOLERANZ_M]
        if not idxs:
            continue

        # Zusammenhängende Durchquerungen trennen: kleine Lücken (Rundung/
        # Messrauschen) tolerieren, ein echter Sprung (z.B. Aufstieg -> später
        # wieder derselbe Höhenbereich beim Abstieg) beginnt eine neue Durchquerung.
        laeufe = []
        lauf = [idxs[0]]
        for idx in idxs[1:]:
            if idx - lauf[-1] <= 2:
        ...
        for li, lauf_idxs in enumerate(laeufe):
            if len(lauf_idxs) <= SEGMENT_SAMPLES:
                gewaehlt = lauf_idxs
            else:
                schritt = (len(lauf_idxs) - 1) / (SEGMENT_SAMPLES - 1)
                gewaehlt = sorted({lauf_idxs[round(j * schritt)] for j in range(SEGMENT_SAMPLES)})

            punkte = [(tp[idx][0], tp[idx][1]) for idx in gewaehlt]
            kms = [km_profil[idx] for idx in lauf_idxs]
            profs = [profil[idx] for idx in lauf_idxs]

            suffix = ""
            if len(laeufe) > 1:
                suffix = " (Hinweg)" if li == 0 else (" (Rückweg)" if li == 1 else f" ({li + 1}. Durchquerung)")

            segmente.append({
                'name': namen[i] + suffix,
                'von': min(profs), 'bis': max(profs),
                'punkte': punkte,
                'ele_min': int(band_von), 'ele_max': int(band_bis),
                'km_von': round(min(kms), 1) if hat_echten_track else None,
                'km_bis': round(max(kms), 1) if hat_echten_track else None,
            })
    return segmente


def segment_label(seg, start_dt=None, duration_h=None):
    # FIX: Bisher nur Name + Höhenband, z.B. "Gipfelbereich (1450-1611 m)". Das
    # sagt nichts darüber, wie weit das noch ist oder wann man dort ist - beides
    # musste man sich aus einem separaten "wann"-Text im Kopf zusammenreimen.
    # Jetzt stehen km-Distanz ab Start und (wenn Start/Dauer bekannt) direkt das
    # Uhrzeitfenster mit im Label.
    teile = [seg['name']]
    if seg.get('ele_min') is not None and seg.get('ele_max') is not None \
            and seg['ele_max'] - seg['ele_min'] > 80:
        hoehe = f"{seg['ele_min']}–{seg['ele_max']} m"
    elif seg.get('ele_max') is not None:
        hoehe = f"ca. {seg['ele_max']} m"
    else:
        hoehe = None

    klammer = []
    if seg.get('km_von') is not None and seg.get('km_bis') is not None:
        klammer.append(f"{seg['km_von']:.1f}–{seg['km_bis']:.1f} km")
    if hoehe:
        klammer.append(hoehe)
    if start_dt and duration_h:
        von_dt, bis_dt = segment_time_window(seg, start_dt, duration_h)
        klammer.append(f"ca. {format_zeit(von_dt)}–{format_zeit(bis_dt)} Uhr")

    if klammer:
        teile.append(f"({', '.join(klammer)})")
    return ' '.join(teile)
...
def analyze_segment_hazards(segmente, now, end_dt, tour_types):
    ...
    FIX (Zeit-Unabhängigkeit): Vorher wurde nur innerhalb des Zeitfensters
    geprüft, das der Zeitplan für den jeweiligen Abschnitt vorsah. Das blendete
    z.B. neu einsetzenden Regen im Tal komplett aus, sobald die
    Tour-Halbzeit erreicht war - der Zeitplan ist aber nur eine Schätzung,
    kein Beweis, dass man dort nicht mehr sein kann (langsameres Tempo,
    Umkehr, Rücksicherung, Abbruch). Jetzt wird jeder Abschnitt unabhängig
    vom Zeitplan bis zum Tourende geprüft.
    ...
    # Fenster gilt jetzt für ALLE Abschnitte gleich: von jetzt (mit kurzer
    # Rückschau gegen Rundungsartefakte) bis zum Tourende.
    von_ges = now - timedelta(minutes=15)
    bis_ges = end_dt
...
def build_segment_alert(segmente, niederschlag, gefahren, start_dt, duration_h,
    ...
    FIX (Zeit-Unabhängigkeit): Abschnitte fielen bisher aus der Kandidatenliste,
    sobald ihr rechnerisch erwartetes Zeitfenster vorbei war ("bereits
    durchschritten"). Damit wurde z.B. neu einsetzender Regen im Tal
    zur Tour-Halbzeit nie gemeldet - der Zeitplan ist eine Schätzung, kein
    Beweis, dass man dort nicht mehr sein kann. Jetzt zählt für JEDEN Abschnitt
    nur noch: Wie lange ist es bis zum vorhergesagten Ereignis (vorlauf), egal
    wo der Abschnitt im ursprünglichen Zeitplan lag."""
    if tour_types is None:
        tour_types = []
    kandidaten = []
    end_dt_utc = (start_dt + timedelta(hours=duration_h)).astimezone(timezone.utc)
    ...
    for si, seg in enumerate(segmente):
        ...
        if gef:
            ...
            rot = gef['score'] >= 60
            # Rot bekommt die volle Restdauer der GESAMTEN Tour als Horizont
            # (nicht nur die des einzelnen Abschnitts), orange nur 4 Stunden.
            horizont = (end_dt_utc - now_utc).total_seconds() / 60 if rot else 240
    ...
    k = kandidaten[0]
    seg = k['seg_obj']
    label = segment_label(seg, start_dt, duration_h)
    ...
    if k['art'] == 'gefahr':
        ...
        if rot:
            titel = f"Kritisch {segment_praep(seg['name'])} – {wann}"
            ...
            titel = f"{grund} {segment_praep(seg['name'])} – {wann}"
        ...
        titel = f"{art.capitalize()} {segment_praep(seg['name'])} – {wann}"
...
def cleanup_old_anonymous_users():
    ...
    state_ref = db.collection('system').document('anon_cleanup')
    state_doc = state_ref.get()
    now_utc = datetime.now(timezone.utc)

    if state_doc.exists:
        last_run_str = state_doc.to_dict().get('last_run')
        if last_run_str:
            last_run_dt = datetime.fromisoformat(last_run_str)
            if (now_utc - last_run_dt).total_seconds() < CLEANUP_INTERVAL_S:
                return  # heute schon gelaufen, noch nicht wieder faellig
...
def check_all_tours():
    ...
    for doc in db.collection('tour_subscriptions').stream():
        ...
        try:
            ...
            segmente = build_segments(track_raw, start_lat, start_lon, peak_lat, peak_lon, peak_alt=peak_alt)
            seg_alert = None
            if segmente:
                try:
            ...
            if now_utc >= next_update_due:
                ...
                if send_hazard_alert:
                    ...
                    # NEU: Wetter-Schnappschuss fuer die Offline-Anzeige in der App. Wird
                    # unabhaengig vom Push-Erfolg gespeichert - beim naechsten Oeffnen der App
                    # mit Empfang automatisch mitgeladen, dank Firestores eingebautem
                    # Offline-Cache bleibt er auch ganz ohne Empfang sichtbar (siehe index.html).
                    update_data['weather_snapshot'] = {
                        'time': now_utc.isoformat(),
                        'stage': current_stage,
                        'summary': multi_loc_status or '',
                        'trend': trend_msg or '',
                    }
