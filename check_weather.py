...
def check_forecast_trend(lat, lon, start_dt, duration):
   try:
       ...
       for i, t_str in enumerate(times):
           # FIX: Zeitzone LOCAL_TZ hart anhängen, damit der Vergleich mit start_dt fehlerfrei klappt
   ...
   for doc in docs:
       ...
       try:
           ...
           if start_dt <= now <= end_dt:
               ...
               # FIX: Verlässlicher, an die Startzeit gebundener Stunden-Timer
               should_run_hourly = False
               if not last_hourly_check:
                   reference_dt = start_dt.astimezone(timezone.utc)
               else:
                   reference_dt = datetime.fromisoformat(last_hourly_check.replace('Z', '+00:00'))

               if (now.astimezone(timezone.utc) - reference_dt).total_seconds() >= 3600:
