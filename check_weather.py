...
def send_high_priority_push(title, body, token):
  """Erzwingt sofortige Zustellung, starke Vibration und erfordert Nutzerinteraktion"""
...
def check_forecast_trend(lat, lon, start_dt, duration, tour_types=None, peak_lat=None, peak_lon=None):
  ...
  try:
      ...
      models_str = "best_match,icon_d2,ecmwf_ifs04,gfs_seamless,geosphere_arome_austria,arome_france"
      url = f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&hourly=temperature_2m,precipitation,snowfall,weather_code,precipitation_probability,wind_gusts_10m,cape,visibility&start_date={date_str}&end_date={end_date_str}&timezone=auto&wind_speed_unit=kmh&models={models_str}"
      ...
      if h:
          for key in list(h.keys()):
              if key.endswith('_best_match'):
                  base_key = key.replace('_best_match', '')
                  h[base_key] = h[key]
      ...
      ph = {}
      if peak_lat and peak_lon:
          p_url = f"https://api.open-meteo.com/v1/forecast?latitude={peak_lat}&longitude={peak_lon}&hourly=temperature_2m,precipitation,snowfall,weather_code,precipitation_probability,wind_gusts_10m,cape,visibility&start_date={date_str}&end_date={end_date_str}&timezone=auto&wind_speed_unit=kmh&models={models_str}"
      ...
      for i, t_str in enumerate(times):
          ...
          if start_dt <= t_dt <= end_dt:
              ...
              if slot_score > max_risk:
                  max_risk = slot_score
                  worst_reasons = list(set(slot_reasons))
...
def check_all_tours():
  ...
  for doc in docs:
      ...
      try:
          ...
          # --- 30-MINUTEN-CHECK FÜR LIVE-GPS VS. INTELLIGENTE ROUTEN-INTERPOLATION ---
          ...
          if updated_at and isinstance(updated_at, datetime):
              ...
              if (now_utc - updated_at.astimezone(timezone.utc)).total_seconds() < 1800:
                  is_gps_stale = False

          if start_dt.astimezone(timezone.utc) <= now_utc <= end_dt.astimezone(timezone.utc) and is_gps_stale:
              if peak_lat and peak_lon and start_lat and start_lon:
                  elapsed_seconds = (now_utc - start_dt.astimezone(timezone.utc)).total_seconds()
                  total_seconds = duration * 3600
                  progress = max(0.0, min(1.0, elapsed_seconds / total_seconds))
                  
                  # Unterscheide Rundweg vs. One-Way (Hüttenwanderung)
                  dist_start_peak = calc_distance_km(float(start_lat), float(start_lon), float(peak_lat), float(peak_lon))
                  is_round_trip = dist_start_peak < 0.5 # Wenn Start und Gipfel fast identisch sind oder kein echter Weg dazwischen
                  
                  if is_round_trip:
                      if progress <= 0.5:
                          path_prog = progress * 2.0  # Aufstieg (0 bis 100%)
                      else:
                          path_prog = 2.0 - (progress * 2.0)  # Abstieg (100 bis 0%)
                  else:
                      path_prog = progress # One-Way: Direkt von Start zu Ziel durchgehend linear
                  
                  lat = float(start_lat) + (float(peak_lat) - float(start_lat)) * path_prog
                  lon = float(start_lon) + (float(peak_lon) - float(start_lon)) * path_prog
          # --------------------------------------------------------
          ...
          if start_dt <= now <= end_dt:
              ...
              if front:
                  ...
                  elif stage in ["early_warning", "update_mid", "update_close"]:
                  ...
                  send_hazard_alert = (alert_title != "" and alert_title != last_alert_title)
              else:
              ...
              if send_hazard_alert and alert_title:
                  try:
                      ...
                      print(f"✅ [Gefahr] Push erfolgreich gesendet! ID: {response}")
                      update_data['last_alert_title'] = alert_title
                  except Exception as fe:
                      print(f"❌ [Gefahr] Firebase Push Fehler: {fe}")
                      if "Unregistered" in str(fe) or "not found" in str(fe) or "registration-token-not-registered" in str(fe):
                          print(f"🗑️ Lösche ungültigen Token für User: {doc.id}")
              ...
              if should_run_hourly:
                  ...
                  body_parts = [p for p in [multi_loc_status, trend_msg] if p]
                  hourly_body = "\n\n".join(body_parts)

                  try:
                      response = send_high_priority_push(hourly_title, hourly_body, token)
                      print(f"✅ [Stündlich] Push erfolgreich gesendet! ID: {response}")
                      update_data['last_hourly_check'] = next_update_due.isoformat()
                  except Exception as fe:
                      print(f"❌ [Stündlich] Firebase Push Fehler: {fe}")
                      if "Unregistered" in str(fe) or "not found" in str(fe) or "registration-token-not-registered" in str(fe):
                          print(f"🗑️ Lösche ungültigen Token für User: {doc.id}")
                          db.collection('tour_subscriptions').document(doc.id).update({'token': None})
