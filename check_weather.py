...
def analyze_advanced_front(lat, lon):
   try:
       ...
       # 2. Erweiterte Radien abfragen: 20km, 16km, 8km, 4km in alle Richtungen
       points = []
       radii = [4, 8, 16, 20]
       for d in LIVE_DIRS:
           for km in radii:
       ...
       for dir_name, items in grouped.items():
           # Wir prüfen das Intervall 20km->16km oder 16km->8km oder 8km->4km für Geschwindigkeitsvektoren
           for outer_km, inner_km in [(20, 16), (16, 8), (8, 4)]:
               if outer_km in items and inner_km in items:
                   ...
                   if o_out and o_in and o_out["time"] and o_in["time"]:
                       ...
                       dt = t_in - t_out
                       if dt > 0:
                           # Berechne Zuggeschwindigkeit in km/h
                           dist_diff = outer_km - inner_km
                           speed_kmh = dist_diff / (dt / 3600)
                           
                           # Realitätsfilter für Wind/Fronten (10 km/h bis 120 km/h Sturmböen)
                           if 10 <= speed_kmh <= 120:
                               # Berechne genaue Ankunftszeit am Standort (0 km)
                               eta_ms = t_in + (inner_km / speed_kmh) * 3600
                               
                               # Bestimme die Stufe basierend auf dem Abstand (20-16km, 16-8km, 8-4km)
                               stage = "early_warning" # 20-16 km
                               if inner_km <= 8:
                                   stage = "update_close" # 8-4 km
                               elif inner_km <= 16:
                                   stage = "update_mid" # 16-8 km
       ...
       if candidates:
           # Wähle den fortgeschrittensten (nächsten) Front-Kandidaten aus
       ...
       try:
           ...
           if start_dt <= now <= end_dt:
               ...
               if front:
                   ...
                   # Gestaffelte Zonen-Logik, damit man nicht zugespammt wird, aber rechtzeitig erfährt:
                   # 1. Zone (20-16 km): Frühe Vorwarnung (z.B. bei schnellem Wind)
                   # 2. Zone (16-8 km): Update-Warnung 
                   # 3. Zone (8-4 km): Letztes Update vor Eintreffen
                   # 4. Zone (0 km): Direkter Einschlag / Eintreffen
                   
                   if stage == "early_warning" and last_notified_stage != "early_warning":
                       current_stage = "early_warning"
                       title = "⚠️ Schlechtwetter im Anmarsch (20km)"
                       body = f"Front zieht{dir_txt} auf und erreicht dich voraussichtlich {time_txt}{speed_txt}."
                       send_alert = True
                   elif stage == "update_mid" and last_notified_stage not in ["update_mid", "update_close", "arrival"]:
                       current_stage = "update_mid"
                       title = "⚠️ Wetterfront rückt näher (12km)"
                       body = f"Das Unwetter ist auf ca. 12 km herangerückt. Ankunft {time_txt}."
                       send_alert = True
                   elif stage == "update_close" and last_notified_stage not in ["update_close", "arrival"]:
                       current_stage = "update_close"
                       title = "⚡ Unwetter kurz vor Erreichen!"
                       body = f"Front nur noch ca. 6 km entfernt! Eintreffen {time_txt}."
                       send_alert = True
                   elif stage == "arrival" and last_notified_stage != "arrival":
                       current_stage = "arrival"
                       title = "🚨 Front hat Position erreicht!"
                       body = "Niederschlag/Gewitter ist direkt über deinem Standort aktiv."
                       send_alert = True
               else:
                   # Entwarnung / Stabiles Wetter
                   if last_state in ['danger', 'worsening', 'early_warning']:
                       current_state = "improving"
                       title = "🌤️ Wetterbesserung"
                       body = "Die Front ist durchgezogen oder löst sich auf. Bedingungen stabilisieren sich."
                       send_alert = (last_notified_stage != "improving")
                   else:
                       current_stage = "stable"
                       last_notified_stage = None # Reset für neue Fronten
               ...
               if send_alert and title:
                   ...
                   print(f"Erweiterter Front-Push gesendet ({current_stage}) an Token: {token[:10]}...")

               db.collection('tour_subscriptions').document(doc.id).update(update_data)

       except Exception as e:
           print(f"Fehler bei erweiterter Front-Analyse: {e}")
