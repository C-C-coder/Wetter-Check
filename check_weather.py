   ...
   for doc in docs:
       ...
       try:
           ...
           if start_dt <= now <= end_dt:
               ...
               # 1. Sofortige Nowcast-Prüfung auf Regen/Gefahr mit optimierten Texten
               if front:
                   ...
                   if stage == "early_warning" and last_notified_stage != "early_warning":
                       ...
                       title = f"⚠️ Schlechtwetter im Anmarsch [{local_time_str}]"
                       body = f"Zieht{dir_txt} auf – Ankunft {time_txt}{speed_txt}.{gps_note}"
                       send_alert = True
                   elif stage == "update_mid" and last_notified_stage not in ["update_mid", "update_close", "arrival"]:
                       current_stage = "update_mid"
                       title = f"⚠️ Unwetter rückt näher [{local_time_str}]"
                       body = f"Entfernung ca. 12 km, zieht{dir_txt} auf (ETA: {time_txt}).{gps_note}"
                       send_alert = True
                   elif stage == "update_close" and last_notified_stage not in ["update_close", "arrival"]:
                       current_stage = "update_close"
                       title = f"⚡ Letzte Warnung (4 km) [{local_time_str}]"
                       body = f"Unwetterfront unmittelbar vor deiner Position! Eintreffen {time_txt}.{gps_note}"
                       send_alert = True
                   elif stage == "arrival" and last_notified_stage != "arrival":
                       current_stage = "arrival"
                       title = f"🚨 Front hat Standort erreicht! [{local_time_str}]"
                       body = f"Niederschlag oder Gewitter ist jetzt direkt aktiv. Suche Schutz!{gps_note}"
                       send_alert = True
               else:
                   if last_state in ['danger', 'worsening', 'early_warning', 'update_mid', 'update_close']:
                       ...
                       body = f"Wetterfront ist abgezogen. Die Bedingungen stabilisieren sich wieder.{gps_note}"
               ...
               # 2. Reguläres stündliches Wetter-Update mit angepasstem Text
               can_hourly_update = False
               if not send_alert:
                   ...
                   if should_run_hourly:
                       can_hourly_update = True
                       send_alert = True
                       title = f"ℹ️ Stündliches Wetter-Update [{local_time_str}]"
                       if current_stage == 'stable':
                           body = f"Bedingungen für deine Tour sind weiterhin stabil.{gps_note}"
                       else:
                           body = f"Leichte Veränderungen in der Nowcast-Lage beobachtet.{gps_note}"
               ...
               if send_alert and title:
                   ...
                   if not can_hourly_update:
                       update_data['last_notified_stage'] = current_stage
                   print(f"Push gesendet an Token: {token[:10]}... | Titel: {title}")
