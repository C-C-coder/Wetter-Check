...
# 9. Hauptschleife für den Cronjob (mit 30-Minuten GPS-Fallback)
def check_all_tours():
 ...
 for doc in docs:
     ...
     try:
         ...
         # --- 30-MINUTEN-CHECK FÜR LIVE-GPS VS. INTERPOLATION ---
         ...
         if start_dt.astimezone(timezone.utc) <= now_utc <= end_dt.astimezone(timezone.utc) and is_gps_stale:
             if peak_lat and peak_lon and start_lat and start_lon:
                 ...
                 lat = float(start_lat) + (float(peak_lat) - float(start_lat)) * progress
                 lon = float(start_lon) + (float(peak_lon) - float(start_lon)) * progress
         # --------------------------------------------------------
         ...
         if start_dt <= now <= end_dt:
             ...
             # BLOCK 1: Gefahren-Warnung senden
             if send_hazard_alert and alert_title:
                 try:
                     ...
                     if "Unregistered" in str(fe) or "not found" in str(fe) or "registration-token-not-registered" in str(fe):
                         print(f"🗑️ Lösche ungültigen Token: {token[:15]}...")
             ...
             # BLOCK 2: Stündliches Update senden (mit sauberem Body-Zusammenbau)
