   ...
   for doc in docs:
       ...
       try:
           ...
           if start_dt <= now <= end_dt:
               ...
               # 1. Sofortige Nowcast-Prüfung auf Regen/Gefahr (läuft in jedem 5-Minuten-Intervall)
               if front:
               ...
               # 2. Reguläres stündliches Wetter-Update (nur alle 60 Minuten, sofern kein akuter Alarm ansteht)
               ...
               if not send_alert:
                   ...
                   if should_run_hourly:
                       ...
                       if current_stage == 'stable':
                           body = f"Stündliches Update: Wetterbedingungen für deine Tour sind stabil und unverändert.{gps_note}"
                       else:
                           body = f"Stündliches Update: Leichte Veränderungen in der Nowcast-Lage beobachtet.{gps_note}"
